(function (angular) {
    ...
    angular.module("app")
        .directive("insertPolicyDirective", ['$state', '$timeout', '$q', 'DataService', 'notify', function (state, timeout, $q, DataService, notify) {

            return {

                templateUrl: currentScriptPath.replace('insertPolicy.js', 'insertPolicy.html'),
                ...
                controller: function ($scope, $location, DataService) {
                    ...
                    $scope.showVendorDD = false;

                    var vendorIDFromURL = $location.search().v_VendorID;
                    if (!vendorIDFromURL)
                    {
                        $scope.showVendorDD = true;
                    }else{
                        $scope.VendorID = vendorIDFromURL;
                    }
                    $scope.fninsertPolicy = function () {
                        if (!$scope.EffectiveDate) {
                            notify.warn("Effective Date cannot be empty.");
                            return;
                        }
                        if (!$scope.ExpiryDate) {
                            notify.warn("Expiry Date cannot be empty.");
                            return;
                        }
